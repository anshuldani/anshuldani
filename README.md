<div align="center">

# Anshul Dani

**Founding AI/ML Engineer at [Levee](https://levee.biz) — building the AI operating system for hotels.**

Production CV pipelines. Agentic systems. Hackathon wins judged by Andrew Ng. Currently shipping spatial capture for hotel digital twins.

[![Portfolio](https://img.shields.io/badge/anshuldani.com-000000?style=for-the-badge&logo=safari&logoColor=white)](https://anshuldani.com)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white)](https://linkedin.com/in/anshul-dani)
[![Email](https://img.shields.io/badge/anshuldani@gmail.com-EA4335?style=for-the-badge&logo=gmail&logoColor=white)](mailto:anshuldani@gmail.com)

</div>

---

### Now shipping

<!--START_NOW-->
- **SpatialClone** — iPhone Pro LiDAR + COLMAP + 3D Gaussian Splatting for hotel digital twins. Marriott demo target.
- **Authflow** — "TurboTax for prior authorization." 3rd place Techstars Chicago Startup Weekend, S26 application in flight.
- **Levee CV pipeline** — 45-class hotel inspection model. Pushed YOLO11m-seg recall from 67% → 73% at 90% precision. Sub-2s inference on GCP.
- **JobOS** — local-first job-search agent. Scrape, tailor, track, send. Because applying to 200 roles by hand is for losers.
<!--END_NOW-->

> Auto-updated by [`.github/workflows/now.yml`](./.github/workflows/now.yml) from `now.md`.

---

### What I actually do

I'm a founding team member at Levee, where I own the computer vision pipeline (YOLO, SAM2, fine-tuned SmolVLM on 2,554 annotated hotel rooms), the agentic stack (LangGraph, Pinecone), and the GCP inference infra. Sub-2-second inference is the bar. We're building the digital twin layer next — 3DGS reconstruction from iPhone Pro captures, ESP32 IoT overlays, generative room previews via WorldGen.

Outside the day job I ship at hackathons. **TAVI** (wearable edge AI for the visually impaired) won Google's Hack for Accessibility 2025. **Arcis** (multi-agent investment intelligence) hit Top 20 at a Financial AI Hackathon judged by Andrew Ng. **ClearCare** (multimodal medical-bill dispute agent) shipped at the Google DeepMind Vibecoding Hackathon. **Authflow** placed 3rd at Techstars Chicago Startup Weekend — we're pushing it toward YC S26.

MS in AI at Illinois Tech, graduating June 2026. F-1 OPT, no sponsorship needed.

### The stack

```
Models      PyTorch · YOLO11 · SAM2 · SmolVLM · Qwen · Gemini · Claude
Agents      LangGraph · Pinecone · MCP · LangChain
Infra       GCP (L4/T4) · Docker · Ray · COLMAP · 3DGS
Frontend    React · Next.js · PyQt · Streamlit
Lang        Python · TypeScript · CUDA · SQL
```

### Selected work

| Project | What it is | Stack |
|---|---|---|
| **[Levee](https://levee.biz)** | AI OS for hotels. CV inspection + digital twins. | YOLO · SAM2 · SmolVLM · GCP |
| **[Authflow](https://github.com/anshuldani)** | Prior auth automation. 3rd at Techstars Chicago. | Multi-agent · FHIR · Next.js |
| **[TAVI](https://github.com/anshuldani)** | Wearable edge AI for the visually impaired. **Google Hack4Access winner.** | Edge ML · CV · Hardware |
| **[Arcis](https://github.com/anshuldani)** | Multi-agent investment intelligence. **Top 20, judged by Andrew Ng.** | LangGraph · Pinecone · GCP |
| **[ClearCare](https://github.com/anshuldani)** | Medical bill dispute agent. | Gemini 2.5 · Multimodal |
| **[JARVIS](https://github.com/anshuldani/jarvis)** | Personal desktop AI — Claude API + Whisper + ElevenLabs. | PyQt · Streaming · Voice |

### Stats

<div align="center">

![Anshul's GitHub stats](https://github-readme-stats.vercel.app/api?username=anshuldani&show_icons=true&theme=transparent&hide_border=true&include_all_commits=true&count_private=true&rank_icon=github)
![Top Langs](https://github-readme-stats.vercel.app/api/top-langs/?username=anshuldani&layout=compact&theme=transparent&hide_border=true&langs_count=8)

![GitHub Streak](https://github-readme-streak-stats.herokuapp.com/?user=anshuldani&theme=transparent&hide_border=true)

![Activity Graph](https://github-readme-activity-graph.vercel.app/graph?username=anshuldani&theme=minimal&hide_border=true&area=true)

</div>

---

<div align="center">

**Hiring or building something interesting?** → [anshuldani@gmail.com](mailto:anshuldani@gmail.com)

*F-1 OPT · No sponsorship needed · Available June 2026*

</div>
